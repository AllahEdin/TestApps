...
namespace WinFormFramework48.Services
{
	public class BlockPipelineTest
	{
		...
		public event Action<PipelineEventArgs> OnStatusChangedEvent;
		...
		public BlockPipelineTest()
		{
			...
			_transformBlock = new TransformBlock<int, Color>(async i =>
			{
				...
				OnStatusChangedEvent?.Invoke(new PipelineEventArgs()
				{
				...
				//Console.WriteLine(
				//	$"finish {i}: Thread: {Thread.CurrentThread.ManagedThreadId.ToString()}: Input {_transformBlock.InputCount} Output {_transformBlock.OutputCount}");
			...
			{
				//Console.WriteLine(
				//	$"==================================> {c.A}: Thread: {Thread.CurrentThread.ManagedThreadId.ToString()} Input {_actionBlock.InputCount}");
